...
INSERT INTO shop_items (name, description, category, price_coins, is_active, image_url, data)
SELECT * FROM (VALUES
  -- Party Dance
...
) AS new_items(name, description, category, price_coins, is_active, image_url, data)
WHERE NOT EXISTS (
  SELECT 1 FROM shop_items WHERE shop_items.name = new_items.name
);
