-- Rankings por modo para separar competicion clasica, arcade y futuras ligas.
-- Los puntajes anteriores a los modos no tienen metadata.gameMode; se tratan como arcade.

CREATE INDEX IF NOT EXISTS idx_snake_scores_mode_score
  ON snake_scores ((COALESCE(NULLIF(metadata->>'gameMode', ''), 'arcade')), score DESC, created_at ASC);
...
BEGIN
  v_mode := CASE
    WHEN mode_filter IN ('classic', 'arcade') THEN mode_filter
    ELSE 'arcade'
  END;
