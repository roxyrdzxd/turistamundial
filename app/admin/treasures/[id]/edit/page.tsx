...
export default function EditTreasurePage() {
  ...
  const loadTreasure = async () => {
    try {
      const response = await fetch(`/api/admin/treasures/${params.id}`)
      const data = await response.json()

      if (data.success && data.treasure) {
        const t = data.treasure
        setTreasure(t)
        setFormData({
          name: t.name,
          ...
          despawn_time: t.despawn_time ? new Date(t.despawn_time).toISOString().slice(0, 16) : '',
          is_active: t.is_active,
          badge_url: t.badge_url || '',
        })
      } else {
        alert('Tesoro no encontrado')
        router.push('/admin/treasures')
      }
      setLoading(false)
    } catch (error) {
      console.error('Error cargando tesoro:', error)
      setLoading(false)
    }
  }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Subir badge si hay archivo nuevo
      if (badgeFile) {
        setUploadingBadge(true)
        const badgeFormData = new FormData()
        badgeFormData.append('file', badgeFile)
        badgeFormData.append('treasureId', params.id as string)
      ...
      if (data.success) {
        ...
        alert('Error al actualizar tesoro: ' + (data.error || 'Error desconocido'))
        setSaving(false)
      }
    } catch (error) {
      console.error('Error actualizando tesoro:', error)
      alert('Error al actualizar tesoro')
      setSaving(false)
    }
  }
      ...
      <AdminLayout>
        <div className="flex items-center justify-center min-h-[400px]">
          <div className="text-center">
            ...
            <p className="text-white/80">Cargando tesoro...</p>
          </div>
        </div>
      </AdminLayout>
    )
    ...
    <AdminLayout>
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold text-white mb-2">Editar Tesoro</h1>
            <p className="text-white/70">Modifica la información del tesoro</p>
          </div>
          <Link
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-lg p-6 border border-white/20 space-y-4">
            ...
            <p className="text-sm text-white/60 mb-4">
              Haz clic en el mapa para cambiar la ubicación del tesoro
            </p>
          ...
          <div className="bg-white/10 backdrop-blur-md rounded-lg p-6 border border-white/20 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-white/80 mb-2">
                {formData.badge_url ? 'Reemplazar Medalla' : 'Subir Medalla'}
              </label>
              <input
                type="file"
          ...
          <div className="flex items-center gap-4">
            ...
            >
              {uploadingBadge ? 'Subiendo medalla...' : saving ? 'Guardando...' : '💾 Guardar Cambios'}
            </button>
            <Link
              href="/admin/treasures"
